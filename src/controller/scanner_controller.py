        ...
        with self._lock:
            ...
            if frame_quality == "LOW_QUALITY":
                ...
                if result.status == "NOK":
                    ...
                    # Un unico frame OK confirma que la racha NOK termino. Se
                    # limpian juntos el streak de seguridad y el contador NOK
                    # visible: ya no se esperan 200 OK para sacar de pantalla
                    # fallas aisladas anteriores. La evidencia grabada y los
                    # contadores historicos de paradas no se modifican.
                    self._nok_streak = 0
                    self._streak_start_mono = None
                    self._ok_count  += 1
                    if self._nok_count > 0:
                        logger.info(
                            f"[{self._id}] contador NOK reseteado a 0 por el "
                            f"primer frame OK (quedaba en {self._nok_count})"
                        )
                        self._nok_count = 0
